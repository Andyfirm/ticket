<template>
  <div id="payment">
    ...
    <div class="paymentMode">
      <div class="wx" @click="wxSelect">
        <div class="icon_wx" v-show="wx === false">
          <img src="/static/images/wx.png" alt>
        </div>
        <div class="icon_wx" v-show="wx === true">
          <img src="/static/images/wx1.png" alt>
        </div>
      </div>
      <div class="zfb" @click="zfbSelect">
        <div class="icon_zfb" v-show="zfb === false">
          <img src="/static/images/zfb.png" alt>
        </div>
        <div class="icon_zfb" v-show="zfb === true">
          <img src="/static/images/zfb1.png" alt>
        </div>
      </div>
      <div class="hyk" @click="hykSelect">
        <div class="icon_hyk" v-show="hyk === false">
          <img src="/static/images/hyk.png" alt>
        </div>
        <div class="icon_hyk" v-show="hyk === true">
          <img src="/static/images/hyk1.png" alt>
        </div>
      </div>
    </div>
    <!-- 二维码 -->
    <div class="qrcodehyk" v-if="!wx && !zfb && !hyk"></div>
    <div class="qrcode" v-if="wx">
      请将微信付款码放置下方扫描窗口
      <img src="/static/images/wxpay.gif" alt>
      <!-- 隐藏域文本输入 -->
      <input type="text" class="hykInput" ref="hykInput" @keyup="autoComplete($event,'wx')">
    </div>
    <div class="qrcode" v-if="zfb">
      请将支付宝付款码放置下方扫描窗口
      <img src="/static/images/Alipay.gif" alt>
      <!-- 隐藏域文本输入 -->
    ...
    <div class="qrcodehyk" v-if="hyk">请将会员卡放到识别区
      ...
      <!-- 隐藏域文本输入 -->
      <input type="text" class="hykInput" ref="hykInput" @keyup="autoComplete($event,'hyk')">
    </div>
    <!-- 说明 -->
    <p class="shuoming">订单已生成，请尽快支付</p>
    <!-- 验证支付结果 -->
    <div class="yanzheng" v-if="yanzheng">正在验证支付结果，请稍后</div>
  </div>
</template>
